...
function processFolder() {
	...
	for (i = 0; i < count; i++) {
		// open raw image
	    open(raw + "/" + raw_list[i]);
	    call('de.unifreiburg.unet.SegmentationJob.processHyperStack', 'modelFilename=' + modeldef + ',Memory (MB):=4000,weightsFilename=' + caffemodel + ',gpuId=GPU 0,useRemoteHost=true,hostname=localhost,port=22,username=sam,RSAKeyfile=/home/sam/.ssh/id_rsa,processFolder=process-folder/,average=none,keepOriginal=false,outputScores=false,outputSoftmaxScores=false');
